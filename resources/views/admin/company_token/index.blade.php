...
<div class="page">
    ...
    <div class="page-wrapper">
        ...
        @yield('content')
        <form id="tokenTable" class ="h-adr"  action="{{route('admin.company_token.store')}}" method="POST">
            @csrf
        ...
        <div class="table-responsive text-nowrap ms-3 p-3 ">
            <table id="sorter" class="table table-bordered tablesorter-bootstrap">
                <thead>
                <tr>
                    <th class="bg-dark text-white" style="font-size: 0.9rem;"  >法人名</th>
                    ...
                    <tr data-company-id="{{ $company->id }}">
                            <td>{{$company->name}}</td>
                                @foreach ($aiInfo as $ai)
                                    @if($ai->enabled == 1)
                                    @php
                                        $token = isset($tokenInfo[$company->id])? $tokenInfo[$company->id]->firstWhere('ai_id', $ai->id) : null;
                                    @endphp
                                    ...
                                    @endif
                        　　　　　@endforeach
                        <td><a href="{{ route('admin.users_token', ['company' => $company->id]) }}" class="btn btn-primary btn-sm">ユーザー別トークン設定</a></td>
        ...
        function validateTokenData(tokenDataJson) {
            ...
            tokenData.forEach(data => {
                ...
                if (data.tokenType === 'input' || data.tokenType === 'output') {
                    if (data.tokenValue === '') {
                        isValid = false;
                        errorMessage = 'cell cannot be blank';
